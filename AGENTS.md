...
| `speech/live.py` | The live engine (ADR 0011): microphone and speakers through PipeWire's echo canceller, Silero VAD, Whisper and Chatterbox, while live mode has the machine; the broker drives it over a private socket, and `den/speech.py` keeps every session's transcript. |
